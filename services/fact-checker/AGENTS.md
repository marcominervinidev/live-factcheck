...
- Responsibility (brief 5, 6): researches each detected claim (web search + page fetch), lets the LLM judge only on the fetched sources and publishes the verdict to `claims.checked` (from phase 1 on); the most expensive worker. In phase 0 it only has the ops endpoints, a Redis connection and a validated LLM configuration.
- From phase 1 on: the SSRF protection and prompt-injection rules of brief 15.5 are mandatory for every fetch and every LLM call.
- Startup and shutdown go through `runService()` from `@lfc/service-kit`; `src/main.ts` stays a thin wiring file.
- Config lives in `src/config.ts` (`baseConfigSchema.extend`). Secrets listed in `secretKeys`: `REDIS_PASSWORD` (via `REDIS_PASSWORD_FILE` in containers) and `CHECKER_LLM_API_KEY` (via `CHECKER_LLM_API_KEY_FILE`).
